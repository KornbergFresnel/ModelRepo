...
class DQN(BaseModel):
    ...
    def act(self, obs, factor=0.):
        ...
        if self.policy_type == 'e_greedy':
            act = np.random.choice(self.act_dim) if policy[0] else np.argmax(value, axis=1)
        elif self.policy_type == 'boltzman':
    ...
    def cal_target(self, next_state, reward, done):
        ...
        res = reward + (1. - np.array(done)) * res * self.gamma
    ...
    def train(self, **kwargs):
        ...
        loss, _ = self.sess.run([self.loss, self.train_op], feed_dict={
            ...
            self.act_ph: np.concatenate(batch.action)
        })
