...
class Actor(BaseModel):
    def __init__(self, env, sess, name, agent_id, config):
        ...
        with tf.variable_scope("actor"):
            ...
            with tf.variable_scope("Optimization"):
                ...
                self.policy_gradients = tf.gradients(self.e_out, self.e_variables, -1.0 * self.q_gradient)
    ...
    def _construct(self, out_dim, name_scope):
        ...
        out = tf.layers.dense(l2, activation=tf.nn.softmax, units=out_dim, kernel_initializer=self.initialize, use_bias=True)
    ...
    def act(self, obs_set):
        ...
        # idx = np.argmax(policy[0])
        #
        # if self.env.discrete_action_input:
        #     action = np.zeros(self.action_space.n, dtype=np.int32)
        #     action[idx] = 1
        # else:
        #     action = np.zeros(self.action_space.n, dtype=np.float32)
        #     action[idx] += 1.
        return policy[0]
    ...
    def target_act(self, obs_set):
        ...
        # obs_set = obs_set.reshape((1,) + obs_set.shape)
        policy = self.sess.run(self.e_out, feed_dict={self.obs_input: obs_set})
        # idx = np.argmax(policy, axis=1)
        #
        # if self.env.discrete_action_input:
        #     action = np.zeros((len(policy), self.action_space.n), dtype=np.int32)
        #     action[idx] = 1
        # else:
        #     action = np.zeros((len(policy), self.action_space.n), dtype=np.float32)
        #     action[idx] += 1.
        return policy
    ...
    def train(self, obs, action_gradients):
        ...
        # variables = self.sess.run(self._variables)
        # if np.isnan(variables[0]).any():  # I don't know how to use RelaxedOneHotCategorical, so duprecate it
        #     input("FUCKYOU FEEFEF")
...
class Critic(BaseModel):
    def __init__(self, env, sess, name, agent_id, config):
        ...
        # self.action_space = env.action_space[agent_id]  # private action_space
        ...
        with tf.variable_scope("critic"):
            ...
            with tf.variable_scope("Optimization"):
                ...
                self.action_gradients = tf.gradients(self.e_q, self.mul_act_input)

    def _construct(self, name_scope):
        ...
        # l3 = tf.layers.dense(l2, units=self.layers_conf[2], activation=self.active_func, name="l3")
        q = tf.layers.dense(l2, units=1, use_bias=False,
                            kernel_initializer=self.initial, name="Q")
    ...
    def train(self, target_q_values, obs, action):
        _, loss, e_q = self.sess.run([self.train_op, self.loss, self.e_q], feed_dict={
            self.mul_obs_input: obs,
            self.mul_act_input: action,
            self.t_q_input: target_q_values.reshape((target_q_values.shape[0], 1)),
        })

        return np.mean(e_q), loss
...
class MultiAgent(object):
    ...
    def act(self, obs_set):
        ...
        for i, obs in enumerate(obs_set):
            actions.append(self.actor[i].act(obs))
        return actions
    ...
    def train(self, replay_buffer):
        ...
        for i_batch in range(n_batch):
            ...
            for j in range(self.env.n):  # calculate Q-value for (s_next, a_next)
                ...
                eval_q, critic_loss = self.critic[j].train(batch_q_value, obs_clus, action_clus)
                # compute action_gradients
                action_gradients[j] = self.critic[j].get_action_graidents(obs_next_clus, batch_next_action)

                self.record.reward[j] += eval_q
                self.record.loss[j] += critic_loss
            
            # reshape action_gradients for each agents
            action_gradients = self.restructure(np.array(action_gradients) / self.env.n)
            ...
            # variables = self.sess.run(self.actor[1].e_variables)
            # if np.isnan(variables[0]).any():
            #     input("Fuck You")
